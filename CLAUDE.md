...
## Xcode-managed file safety (reinforced 2026-06-12 — **sixth pass**, user-direct, auto-cycle round)

**Critical** (verbatim user-direct, 2026-06-12 — now repeated across **SIX** reinforcement passes the same day, this time again paired with the standing auto-cycle directive `branch → commit → push → gh pr create → gh pr merge → verify` for multi-commit work AND an explicit instruction to maximize ForgeKit integration + FEATURE_PLAN.md closure AND an explicit instruction to verify SPM module folder structure): *"critical: do not author/edit xcode-managed files including Xcode workspace file and Xcode scheme/test plan file. staging and committing is ok."* Six restatements in one calendar day is the strongest cadence the portfolio has recorded for any rule. The cadence is itself the signal: this is THE most load-bearing safety rule for the in-IDE agent, and any auto-cycle multi-PR session must NOT relax it.

**Six-pass invariant** (codified after the sixth restatement; supersedes the five-pass invariant): when a single safety rule is restated **six** times in one calendar day by the same user, every subsequent session in this repo treats it as an **immutable pre-flight check**, not a guideline — and every subsequent session in this repo MUST surface a rule-restatement summary at the top of any handoff doc the round produces (so the next round inherits the cadence without re-reading the entire CLAUDE.md). Before any `Edit` / `Write` tool call against a path that matches the table at the top of this file, STOP. No exceptions. The auto-cycle does NOT extend to managed-file edits; the maximize-ForgeKit-integration directive does NOT extend to managed-file edits; the close-FEATURE_PLAN-checkboxes directive does NOT extend to managed-file edits; the "follow technical design doc" directive does NOT extend to managed-file edits; the "swift files follow standard SPM folder structure" directive does NOT extend to managed-file edits. Every checkbox in FEATURE_PLAN.md that conceptually requires a managed-file edit ships via `Docs/HANDOFF_TO_USER_<TOPIC>.md` describing the Xcode GUI steps — never an attempted direct edit.
...
**SPM folder-structure interaction note** (new for the 6th pass): "make sure swift files in SPM modules follow standard folder structure" applies to `Packages/Libraries/Sources/<Target>/` only — that's where the agent legitimately operates. The instruction does NOT authorize moving / renaming / reorganizing files in app-target paths owned by the Xcode project (`MicrobeLab/MicrobeLabApp.swift` + `MicrobeLab/Assets.xcassets/` etc.), which would force `.pbxproj` regeneration. SPM source reorganization is FREE (no Xcode project membership to update — see `@.claude/rules/spm-architecture.md` § Key Rules); app-target reorganization is OFF-LIMITS to the agent regardless of how clean the proposed structure is.
