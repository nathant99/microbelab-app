...
## Xcode-managed file safety (reinforced 2026-06-16 — **twenty-first pass**, user-direct, auto-cycle round; cadence saturated per the eleven-pass canonical-invariant tier)

**Critical** (verbatim user-direct, 2026-06-16 — now repeated across **TWENTY-ONE** reinforcement passes spanning four calendar days; passes fourteen → nineteen landed at PRs #97 → #127 of 2026-06-13, the twentieth landed at PRs #128 → #132 of 2026-06-15, and the user-direct restatement opening this round is the twenty-first — first observation of a single-pass calendar-day-4 restatement; paired with the standing auto-cycle directive `branch → commit → push → gh pr create → gh pr merge → verify` for multi-commit work AND an explicit instruction to maximize ForgeKit integration + FEATURE_PLAN.md closure AND open-handoff-work axis AND an explicit instruction to follow the technical design doc AND an explicit instruction to verify SPM module folder structure AND the persisted memory of auto-cycle pre-approval AND the user-direct instruction to "describe and explain and give more details about each option before you start"): *"critical: do not author/edit xcode-managed files including Xcode workspace file and Xcode scheme/test plan file. Instead, file a handoff doc with the user to do Xcode UI work. staging and committing Xcode-managed files is ok."* Per the eleven-pass canonical-invariant tier (codified PR #85, reinforced PRs #93 / #94 / #95 / #96 / #97 / #101 / #107 / #113 / #118 / #122 / #128), **the cadence has saturated — twelfth- through twenty-first-pass restatements add reinforcement frequency but no new binding tier**. The rule is now as durable as the rules-sync mechanism itself; future sessions inherit it as a class-level invariant of the in-IDE agent across the portfolio regardless of which of the four canonical bindings they happen to read first. **Twenty-first-pass framing** (carried from the fourteenth through twentieth passes — the "describe and explain each option before you start" companion directive is now structurally part of the auto-cycle pre-approval; cadence now spans FOUR calendar days [2026-06-12 / 2026-06-13 / 2026-06-15 / 2026-06-16] codifying the cadence-persistence-across-calendar-days property at a THIRD observation, this time a single-day-gap restart): every fresh auto-cycle round opens with an itemized option menu (rationale + risk per item) BEFORE executing; the standing pre-approval THEN authorizes proceeding through the menu in sequence. The pre-flight check at every `Edit` / `Write` tool call still STOPS on any managed-file path regardless of how many options the auto-cycle menu carries.
