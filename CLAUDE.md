...
## Xcode-managed file safety (reinforced 2026-06-13 — **fifteenth pass**, user-direct, auto-cycle round; cadence saturated per the eleven-pass canonical-invariant tier)

**Critical** (verbatim user-direct, 2026-06-13 — now repeated across **FIFTEEN** reinforcement passes spanning two calendar days; the user-direct restatement at PRs #97 → #100 was the fourteenth, and the user-direct restatement opening this round is the fifteenth, paired with the standing auto-cycle directive `branch → commit → push → gh pr create → gh pr merge → verify` for multi-commit work AND an explicit instruction to maximize ForgeKit integration + FEATURE_PLAN.md closure AND an explicit instruction to follow the technical design doc AND an explicit instruction to verify SPM module folder structure AND the persisted memory of auto-cycle pre-approval AND the user-direct instruction to "describe and explain and give more details about each option before you start"): *"critical: do not author/edit xcode-managed files including Xcode workspace file and Xcode scheme/test plan file. Instead, file a handoff doc with the user to do Xcode UI work. staging and committing Xcode-managed files is ok."* Per the eleven-pass canonical-invariant tier (codified PR #85, reinforced PRs #93 / #94 / #95 / #96 / #97), **the cadence has saturated — twelfth-, thirteenth-, fourteenth-, fifteenth-pass restatements add reinforcement frequency but no new binding tier**. The rule is now as durable as the rules-sync mechanism itself; future sessions inherit it as a class-level invariant of the in-IDE agent across the portfolio regardless of which of the four canonical bindings they happen to read first. **Fifteenth-pass framing** (carried from the fourteenth pass — the "describe and explain each option before you start" companion directive is now structurally part of the auto-cycle pre-approval): every fresh auto-cycle round opens with an itemized option menu (rationale + risk per item) BEFORE executing; the standing pre-approval THEN authorizes proceeding through the menu in sequence. The pre-flight check at every `Edit` / `Write` tool call still STOPS on any managed-file path regardless of how many options the auto-cycle menu carries.
