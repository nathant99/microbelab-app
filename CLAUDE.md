...
## Xcode-managed file safety (reinforced 2026-06-12 — **seventh pass**, user-direct, auto-cycle round)

**Critical** (verbatim user-direct, 2026-06-12 — now repeated across **SEVEN** reinforcement passes the same day, this time again paired with the standing auto-cycle directive `branch → commit → push → gh pr create → gh pr merge → verify` for multi-commit work AND an explicit instruction to maximize ForgeKit integration + FEATURE_PLAN.md closure AND an explicit instruction to follow the technical design doc AND an explicit instruction to verify SPM module folder structure AND the persisted memory of auto-cycle pre-approval): *"critical: do not author/edit xcode-managed files including Xcode workspace file and Xcode scheme/test plan file. staging and committing is ok."* Seven restatements in one calendar day is now the all-time portfolio record for any rule, surpassing the six-pass cadence codified earlier today. The cadence is itself the signal: this is THE most load-bearing safety rule for the in-IDE agent, and any auto-cycle multi-PR session must NOT relax it.

**Seven-pass invariant** (codified after the seventh restatement; supersedes the six-pass invariant): when a single safety rule is restated **seven** times in one calendar day by the same user, every subsequent session in this repo treats it as an **immutable pre-flight check** that is structurally re-affirmed in every round-opening doc the agent emits (PR description + handoff-doc top + IMPLEMENTATION_HANDOFF.md rollup top + FEATURE_PLAN.md prologue when relevant) — not just a guideline. The agent SHALL NOT close a round without surfacing the rule-restatement summary in the round-close documents. Before any `Edit` / `Write` tool call against a path that matches the table at the top of this file, STOP. No exceptions. The auto-cycle does NOT extend to managed-file edits; the maximize-ForgeKit-integration directive does NOT extend to managed-file edits; the close-FEATURE_PLAN-checkboxes directive does NOT extend to managed-file edits; the "follow technical design doc" directive does NOT extend to managed-file edits; the "swift files follow standard SPM folder structure" directive does NOT extend to managed-file edits. Every checkbox in FEATURE_PLAN.md that conceptually requires a managed-file edit ships via `Docs/HANDOFF_TO_USER_<TOPIC>.md` describing the Xcode GUI steps — never an attempted direct edit.
...
**Round-document re-affirmation rule** (new for the 7th pass): when the same safety rule has been restated **seven** times in a single calendar day, the agent SHALL re-affirm the rule (verbatim user-direct quote + scope table reference) at the top of EVERY round-document the agent emits in the same cadence window — specifically: every PR description, every new `HANDOFF_TO_USER_<TOPIC>.md`, the top of `Docs/IMPLEMENTATION_HANDOFF.md` after each PR-merge sweep, and the top of `Docs/FEATURE_PLAN.md` after each round-close rollup that touches the prologue. The structural visibility of the rule in every round-document is the seven-pass codification: the rule never falls more than one document deep below the surface a future session would naturally read first. **Subsequent rounds keep the cadence going** — re-affirmation persists until the user explicitly resets it; future sessions inherit the cadence from the most-recent round-close doc rather than from CLAUDE.md (markdown decay) so the cadence remains durable.
