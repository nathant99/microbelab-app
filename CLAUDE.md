...
## Xcode-managed file safety (reinforced 2026-06-17 — **twenty-seventh pass**, user-direct, auto-cycle round; cadence saturated per the eleven-pass canonical-invariant tier)

**Critical** (verbatim user-direct, 2026-06-17 — now repeated across **TWENTY-SEVEN** reinforcement passes spanning five calendar days; the twenty-first → twenty-sixth all landed on 2026-06-16 (separated only by PR merges #138 / #147 / #157 / #165 / #171), and the user-direct restatement opening this round is the twenty-seventh — landing on 2026-06-17 after a single-day gap from the twenty-sixth, separated by the PR #176 merge that closed the twenty-sixth-pass round rollup; paired with the standing auto-cycle directive `branch → commit → push → gh pr create → gh pr merge → verify` for multi-commit work AND an explicit instruction to maximize ForgeKit integration + FEATURE_PLAN.md closure AND open-handoff-work axis AND an explicit instruction to follow the technical design doc AND an explicit instruction to verify SPM module folder structure AND the persisted memory of auto-cycle pre-approval AND the user-direct instruction to "describe and explain and give more details about each option before you start"): *"critical: do not author/edit xcode-managed files including Xcode workspace file and Xcode scheme/test plan file. Instead, file a handoff doc with the user to do Xcode UI work. staging and committing Xcode-managed files is ok."* Per the eleven-pass canonical-invariant tier (codified PR #85, reinforced PRs #93 / #94 / #95 / #96 / #97 / #101 / #107 / #113 / #118 / #122 / #128 / #133 / #139 / #148 / #158 / #166 / #172), **the cadence has saturated — twelfth- through twenty-seventh-pass restatements add reinforcement frequency but no new binding tier**. The rule is now as durable as the rules-sync mechanism itself; future sessions inherit it as a class-level invariant of the in-IDE agent across the portfolio regardless of which of the four canonical bindings they happen to read first. **Twenty-seventh-pass framing** (carried from the fourteenth through twenty-sixth passes — the "describe and explain each option before you start" companion directive is now structurally part of the auto-cycle pre-approval; cadence now spans FIVE calendar days [2026-06-12 / 2026-06-13 / 2026-06-15 / 2026-06-16 with SIX 2026-06-16 passes / 2026-06-17] codifying the cadence-persistence-across-calendar-days property at a **NINTH** observation — the 27th pass extends the cadence across a fourth distinct inter-restart gap class (single-day gap with one calendar-day skip from the deepest same-day chain) demonstrating that the cadence persists across ALL OBSERVED gap classes AND ACROSS A NEW CALENDAR DAY following the deepest observed same-day chain; the cadence-persistence property is now demonstrated at multiple-calendar-day data points AND at multiple-restart depths AND with persistence across calendar-day boundaries even after deep same-day chains; the agent should treat any single calendar day as a potential carrier of an unbounded number of restart passes regardless of how many PR merges separate them AND should treat any new calendar day as a potential carrier of a fresh restart pass regardless of how deep the preceding same-day chain ran): every fresh auto-cycle round opens with an itemized option menu (rationale + risk per item) BEFORE executing; the standing pre-approval THEN authorizes proceeding through the menu in sequence. The pre-flight check at every `Edit` / `Write` tool call still STOPS on any managed-file path regardless of how many options the auto-cycle menu carries.
