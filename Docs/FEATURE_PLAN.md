...
- [/] **Mastery moments** — Distinct screen ripple + chord when child internalizes microbiome ecology. **Ecology mastery axis shipped via PR #76** (eighth-pass auto-cycle sweep). `MasteryMomentDetector` (Services/Engagement/, pure nonisolated `Sendable` value type) tracks per-session mastery thresholds for `.ecologyMaster` / `.defenseMaster` / `.codexMaster` with `acknowledged: Set<MasteryKind>` so each kind fires once per session. Threshold `MasteryMomentDetector.ecologyMasteryStableTickThreshold` = 15 consecutive stable ticks under fiber feeding (≥ 3 stability-milestone events; demonstrates feeding-mode → growth-rate causality internalization). `MicrobiomeView` carries the detector as `@State`, fires `evaluateEcologyMastery()` after every tick; on a returned `Moment` it: (a) fires `CelebrationCoordinator.personalBest(metric:value:)` (`.epic` tier ripple + chord via the `daily-complete` Lottie), (b) carries the moment's subline into the mentor bubble for calm narration, (c) fires `SensoryPaletteCoordinator.streakMilestone(threshold)` for distinct haptic. Detector also surfaces `recordDefenseRunComplete(wavesCleared:pathogensRemaining:)` (perfect 5-wave run) and `recordCodexDiscovery(totalDiscovered:totalAvailable:)` (12/12 microbes) for follow-up wiring in `ImmuneGameView` + `MicrobeCodexView`. Trauma-informed: copy stoplist (`finally` / `at last` / `you almost` / `failed` / `behind` / `should have` / `compared to` / `better than`) pinned by parameterized test across all three moment kinds. 12 new `MasteryMomentDetectorTests` pin per-kind threshold + one-shot behavior + acknowledge idempotency + trauma-safe register.
