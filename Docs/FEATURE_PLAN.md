...
> **Ninth-pass rule-restatement summary** (top-of-doc per the nine-pass invariant codified 2026-06-12; verbatim user-direct, repeated NINE times in one calendar day — all-time portfolio record, surpassing the eight-pass cadence codified earlier the same day which itself surpassed the seven-pass cadence codified earlier the same day): *"critical: do not author/edit xcode-managed files including Xcode workspace file and Xcode scheme/test plan file. staging and committing is ok."* Scope: `*.xcworkspace/contents.xcworkspacedata` / `*.xcodeproj/project.pbxproj` / `*.xcscheme` / `*.xctestplan` / `Info.plist` / `*.entitlements` / `*.xcassets/Contents.json` / `xcuserdata/` / `Package.resolved`. No checkbox below implies an edit to any of those paths; items that need them ship via `Docs/HANDOFF_TO_USER_<TOPIC>.md`. The 9th-pass invariant graduates the rule into **load-bearing identity-of-the-agent tier** — the rule is no longer merely a thing the agent inherits, it is a thing that defines what "being the MicrobeLab in-IDE agent" means. Persisted in the agent's memory file (`feedback_xcode_managed_files.md`) so future sessions inherit the constitutional check automatically. See `@CLAUDE.md` § Xcode-managed file safety for the canonical statement.
>
...
- [/] **Share-worthy moments** — Codex completion certificate axis shipped via **PR #75** (eighth-pass auto-cycle sweep). `CodexCertificate` (Services/Engagement/, pure nonisolated `Sendable` value type) snapshots the kid's discovery progress at the moment they tap "Share my codex"; future-session activity doesn't retroactively change a shared certificate. Headline scales warmly with progress (`Microbe Explorer Pass` → `Microbe Field Notebook` → `Microbe Naturalist` → `Microbe Scientist` → `Codex Complete!`); subline never benchmarks the kid against anyone else; trauma-informed stoplist (`missed` / `failure` / `behind` / `should` / `must`) pinned by `CodexCertificateTests`. `CodexCertificateView` (AppFeature/Engagement/) renders the card; `CodexCertificateSheet` wraps it with a `ShareLink` that pipes an `ImageRenderer`-produced PNG through the system share sheet (Messages / Mail / AirDrop / Save Image — MicrobeLab never touches those surfaces directly). Sheet copy frames sharing as quiet recognition, NEVER engagement pressure ("share to earn XP" / "your friends are sharing"). `ProgressTabView` gains a secondary-action toolbar "Share my codex" button that presents the sheet. 8 new `CodexCertificateTests` pin headline tiers + singular/plural copy + Codable roundtrip + trauma-informed copy invariants. Immune-game high-score trophy axis still pending.
