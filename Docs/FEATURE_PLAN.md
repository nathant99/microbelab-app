...
> **Eleventh-pass rule-restatement summary** (top-of-doc per the eleven-pass invariant codified 2026-06-12; verbatim user-direct, repeated ELEVEN times in one calendar day — all-time portfolio record, surpassing the ten-pass cadence codified earlier the same day which itself surpassed the nine-pass cadence codified earlier the same day which itself surpassed the eight-pass cadence codified earlier the same day which itself surpassed the seven-pass cadence codified earlier the same day): *"critical: do not author/edit xcode-managed files including Xcode workspace file and Xcode scheme/test plan file. staging and committing is ok."* Scope: `*.xcworkspace/contents.xcworkspacedata` / `*.xcodeproj/project.pbxproj` / `*.xcscheme` / `*.xctestplan` / `Info.plist` / `*.entitlements` / `*.xcassets/Contents.json` / `xcuserdata/` / `Package.resolved`. No checkbox below implies an edit to any of those paths; items that need them ship via `Docs/HANDOFF_TO_USER_<TOPIC>.md`. The 11th-pass invariant crosses the rule past the doctrinal tier (10th pass) into the **canonical-invariant tier** — a class-level invariant of the in-IDE agent across the portfolio, not just this codebase. Beyond the eleven-pass tier the cadence has saturated: twelfth-, thirteenth-pass restatements add reinforcement frequency but no new binding tier. The rule lives in FOUR canonical locations (CLAUDE.md + `.claude/rules/xcode-agent-safety.md` + this prologue + the persistent-memory file `feedback_xcode_managed_files.md`) so any one of them can drift or decay without the rule being lost — the other three carry it forward. See `@CLAUDE.md` § Xcode-managed file safety for the canonical statement.
>
