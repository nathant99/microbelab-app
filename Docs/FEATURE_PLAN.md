...
> **Eighth-pass rule-restatement summary** (top-of-doc per the eight-pass invariant codified 2026-06-12; verbatim user-direct, repeated EIGHT times in one calendar day — all-time portfolio record, surpassing the seven-pass cadence codified earlier the same day): *"critical: do not author/edit xcode-managed files including Xcode workspace file and Xcode scheme/test plan file. staging and committing is ok."* Scope: `*.xcworkspace/contents.xcworkspacedata` / `*.xcodeproj/project.pbxproj` / `*.xcscheme` / `*.xctestplan` / `Info.plist` / `*.entitlements` / `*.xcassets/Contents.json` / `xcuserdata/` / `Package.resolved`. No checkbox below implies an edit to any of those paths; items that need them ship via `Docs/HANDOFF_TO_USER_<TOPIC>.md`. The 8th-pass discipline also persists in the agent's memory file (`feedback_xcode_managed_files.md`) so future sessions inherit the cadence specification automatically. See `@CLAUDE.md` § Xcode-managed file safety for the canonical statement.
>
